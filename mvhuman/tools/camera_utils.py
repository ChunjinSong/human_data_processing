...
def read_camera_mvhumannet(intri_name, extri_name, cam_scale_name, img_scale_factor=1.0):
    camera_scale = pickle.load(open(cam_scale_name, 'rb'))
    img_scale_factor = 0.5 * img_scale_factor
    if camera_scale == 120 / 65:
        img_size = (np.array([4096, 3000]) * img_scale_factor).astype(np.int16)
    else:
        img_size = (np.array([2448, 2048]) * img_scale_factor).astype(np.int16)
    ...
    for cam in cam_names:
        ...
        cams[updated_cam]['K'][:2] *= img_scale_factor
