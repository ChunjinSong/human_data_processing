...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        joints_return_type='vertex_joint_selector'
    ) -> SMPLOutput:
        ...
        if displacement is not None:
            if absolute_displacement:
                vertices, joints, A, J = lbs(betas, full_pose, displacement,
                                    self.shapedirs, self.posedirs,
                                    self.J_regressor, self.parents,
                                    self.lbs_weights, pose2rot=pose2rot)
            else:
                vertices, joints, A, J = lbs(betas, full_pose, self.v_template+displacement,
                                    self.shapedirs, self.posedirs,
                                    self.J_regressor, self.parents,
                                    self.lbs_weights, pose2rot=pose2rot)
        else:
            vertices, joints, A, J = lbs(betas, full_pose, self.v_template,
                                self.shapedirs, self.posedirs,
                                self.J_regressor, self.parents,
                                self.lbs_weights, pose2rot=pose2rot)
        if joints_return_type == 'smpl':
            pass
        else:
