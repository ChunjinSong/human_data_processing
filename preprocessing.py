...
def main(args):
    ...
    if args.source == 'custom':
        ...
        cam_intrinsics = np.array([[float(cam_params[4]), 0., float(cam_params[6])], 
                                   ...
                                   [0., 0., 1.]])
    elif args.source == 'mocap':
        MOCAP_DIR = ' '  # path to Mocap dataset
        annots = np.load(f'{MOCAP_DIR}/annots.npy', allow_pickle=True).item()
        cams = annots['cams'][0]
        cam_intrinsics = np.array(cams['K'])[0].astype('float32')

    elif args.source == 'aist':
        AIST_DIR = ' '  # path to aist dataset
        mapping_seq2env = {}  # sequence name -> env name
        env_mapping_file = os.path.join(AIST_DIR, 'annotation/cameras/mapping.txt')
        env_mapping = np.loadtxt(env_mapping_file, dtype=str)
        for seq_name, env_name in env_mapping:
            mapping_seq2env[seq_name] = env_name

        env_name = mapping_seq2env[seq.replace('c01', 'cAll')]
        file_path = os.path.join(AIST_DIR, f'annotation/cameras/{env_name}.json')
        ...
        cam_intrinsics = np.array(intrinsics)[0].astype('float32')
    ...
    for idx, img_path in enumerate(tqdm(img_paths)):
        ...
        if args.mode == 'mask' or args.mode == 'refine':
            if args.source == 'mocap':
                mocap_smpl_dir = f'{DIR}/{seq}/new_params'
                smpl_file_paths = sorted(glob.glob(f"{mocap_smpl_dir}/*.npy"))
                seq_file = np.load(smpl_file_paths[idx], allow_pickle=True)['results'][()]
            else:
...
if __name__ == '__main__':
    ...
    # sequence name
    parser.add_argument('--seq', type=str, default='story_training')
    # gender
    parser.add_argument('--gender', type=str, default='MALE', help="gender of the actor: MALE or FEMALE")
    # mode
    parser.add_argument('--mode', type=str, default='refine', help="mask mode or refine mode: mask or refine or final")
