...
from volumetric_dataset import VolumetricDataset, VolumetricDatasetFilepaths
...
def download_dataset(
    ...
    download_lazy(
        links[actor][sequence]["smpl"],
        dataset_paths.smpl_path,
    )
    download_lazy(
        links[actor][sequence]["smplx"],
        dataset_paths.smplx_path,
    )

    download_lazy(
        links[actor][sequence]["scene_blend"],
        dataset_paths.scene_path,
    )
...
def main():
    ...
    parser.add_argument("--dataset_file", default='./config/actorshq_access_4x.yaml', type=Path)
    # parser.add_argument("--target", default='../../raw_data/actorshq', type=Path)
    # parser.add_argument("--target", default='/media/chunjins/My Passport/project/HumanNeRF/0_dataset/Actor-HQ', type=Path)
    parser.add_argument("--target", default='../../raw_data/actorshq_smpl', type=Path)
    parser.add_argument("--actor", default='Actor01', choices=[f"Actor{i:02d}" for i in range(1, 9)])
    parser.add_argument("--sequence",  default='Sequence2', choices=["Sequence1", "Sequence2"])
    ...
    parser.add_argument("--frame_stop", type=int, default=0, help="Last frame to download (exclusive). 0 means all frames.")
    parser.add_argument(
        "--include",
        default=["rgb", "mask", "mesh"],
